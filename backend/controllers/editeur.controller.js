...
export const updateEditeur= async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`pas de éditeur avec un id: ${id}`);

    const edi1 = { maisonedit:maisonedit, siteweb:siteweb, email:email, _id: id };
...
export const deleteEditeur = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`pas de éditeur avec l'ID: ${id}`);
