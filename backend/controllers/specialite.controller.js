...
export const updateSpecialite= async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`pas de specialité avec un id: ${id}`);
...
export const deleteSpecialite = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`pas de specialité avec l'ID: ${id}`);
